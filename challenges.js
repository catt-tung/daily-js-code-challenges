...
// if typeof x ==! num and typeof y ==! num 
//   return "NaN"
// else
//   return x + y
...
//reduce function to sum up numbers in an array

// Your solution for 03-sumNumbers here:

function sumNumbers(arr){
  return arr.reduce((prv, cur) => prv + cur, 0)
}
...
function computeRemainder (end, divis){
  if (divis === 0) {
    ...
    let rem = end - Math.floor(end/divis)*divis
    return rem
  }
}
...
//use .reverse() and .toUpperCase() methods
...
//create an array with the length of the padding by getting padding - stringlength
//add the integer to the end of the array 
//join it as a string
// function formatWithPadding(int, str, pad){
//   let arr = new Array((pad - str.length)).fill(str);
//   arr.push(int).join();
...
//isolate character at index i, repeat that character i times
function mumble(str){
  let result = []
  for (let i=0; i < str.length; i++){
    result.push(str.charAt(i).repeat(i+1))
  }
  return result.join('-')
}
...
//access the price property from the object - access the value in the second element of the object
...
function reduceArray(arr, cbf, firstAcc){
  let acc = firstAcc;
  arr.forEach((elem, idx) => {
    acc = cbf(acc, elem, idx);
  })
  return acc
}
